...
def prepare_emocci_filter(em_filter, dep_index_list, field_angle_list, field_angle_range, fov_width):
    ...
    if num_slits % 2 == 0:
        num_slits -= 1
    half_slits = num_slits // 2
    center_slit = (end_slit_index - begin_slit_index) // 2 + begin_slit_index

    begin_slit_index = center_slit - half_slits * fov_width - half_fov
    end_slit_index = center_slit + half_slits * fov_width + half_fov
    ...
    for dep_i in range(len(em_filter)):
        ...
        for slit_num in range(
            center_slit - (half_slits * fov_width),
            center_slit + (half_slits * fov_width) + 1,
            fov_width,
        ):
